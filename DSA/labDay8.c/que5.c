...
void infixToPrefix_198(char *exp)
{
    int i, k;
    strrev(exp);

    // create a st of capacity equal to expression size
    ...
    for (i = 0, k = -1; exp[i]; ++i)
    {
        ...
        // If the scanned character is an '(',push it to the st.
        else if (exp[i] == ')')
            push_198(st, exp[i]);

        // If the scanned character is an ')',pop_198 and output from the st until an '(' is encountered.
        else if (exp[i] == '(')
        {
